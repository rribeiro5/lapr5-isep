...
namespace Tests.TestesIntegracao.UC28_Consultar_Tag_clouds_relaçao_proprio
{
    public class TagCloudControllerServiceTest
    {
        ...
        {
            _mock.Setup(x => x.BidirecionalConnectionsOfUser(It.IsAny<UserId>()))
                .Returns(Task.FromResult<List<Connection>>(null));
            var result = _controller.GetAllUserConnectionsTagCloud(_idList[0].AsGuid()).Result;
        ...
        public void ReturnOkFoundWhenTagCloudIsFound()
        {
            ...
            _mock.Setup(x => x.BidirecionalConnectionsOfUser(u1))
                .Returns(Task.FromResult(connList));
            var result = _controller.GetAllUserConnectionsTagCloud(u1.AsGuid()).Result;
        ...
        public void ReturnExpectedTagCloudWhenTagCloudIsFound()
        {
            ...
            _mock.Setup(x => x.BidirecionalConnectionsOfUser(u1))
                .Returns(Task.FromResult(connList));
            var response = _controller.GetAllUserConnectionsTagCloud(u1.AsGuid()).Result as OkObjectResult;
        ...
        {
            _mock.Setup(x => x.BidirecionalConnectionsOfUser(_idList[0]))
