...
namespace Tests.TestesUnitarios.UseCasesTests.UC28_Consultar_Tag_clouds_relaçao_proprio
{
    public class TagServiceTest
    {
        ...
        public void SuccessfullyGetListOfTagClouds()
        {
            ...
            var result = _service.GetAllUserConnectionsTagCloud(u1.AsGuid()).Result;
        ...
        public void SuccessfullyReturnNullWhenListIsNull()
        {
            ...
            var result = _service.GetAllUserConnectionsTagCloud(_idList[0].AsGuid()).Result;
            Assert.Null(result);
        }

        private Connection CreateConnection(int u1Index, int u2Index, params int[] tagsIndices)
        {
            var u1 = _idList[u1Index];
            var u2 = _idList[u2Index];
            var tags = new List<string>();
            foreach (var t in tagsIndices)
            {
                tags.Add(_tagList[t]);
            }
            return new Connection(1, 1, tags, u1, u2);
        }
